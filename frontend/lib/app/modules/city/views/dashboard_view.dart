...
import 'package:frontend/app/modules/city/controllers/city_controller.dart';
...
class _DashboradState extends State<Dashborad> {
  ...
  void getSites() async {
    var x = await GetIt.I<APIServices>().listSites(page: 0, limit: 15);
    setState(() {
      x.add(newSite);
      sites = x;
    });
  ...
  void setSelectedSiteLen() async {
    var x = await GetIt.I<APIServices>().listSites(page: 0, limit: 15);
    setState(() {
      selectedSite = List.filled(x.length + 1, false);
    });
  }
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        return GestureDetector(
                          child: SiteSelector(
                            siteName: sites[index].id,
                            siteImageURL:
                                'assets/icons/${sites[index].id!}.svg',
                            isSelected: selectedSite[index],
                          ),
                          onTap: () async {
                            if (index == sites.length - 1) {
                              newSiteName = await showTextInputDialog(
                                context: context,
                                textFields: const [
                              ...
                              sites.add(Site(sensors: [], id: newSiteName));
                              selectedSite = List<bool>.filled(
                                  sites.length, false,
                                  growable: true);
                              selectedSite.add(true);
                            } else
                ...
                OutlinedButton(
                    ...
                    onPressed: () {
                      ...
                      GetIt.I<APIServices>().addConnectedObject(
                          siteId: siteId, objectId: sensorName, pin: sensorPin);
                    }),
                SizedBox(
                  height: 100,
                )
...
class DashboardView extends GetView<CityController> {
  @override
  Widget build(BuildContext context) {
    Size size = Get.size;
    return Dashborad();
